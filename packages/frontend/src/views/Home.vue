<template>
  <div>
    <div id="header-bar" :class="[(scrollState.atTop ? 'heightTall' : 'heightShort elevation-5'), 'pa-4']">
      <a href="#top">
        <transition mode="out-in">
          <v-img v-if="scrollState.mainLogoIsVisible"
        ...
        <div class="d-flex flex-wrap justify-end align-center">
          <template v-if="!isAuthenticated">
            <v-btn href="https://chrome.google.com/webstore/detail/cfeaabebicbbcmddmgphgncpdlkadgfl?authuser=2&hl=en"
              target="_blank"
              rounded
          ...
          <template v-else>
            <span class="secondary--text mx-1 px-4">Welcome, {{ firstName }}</span>
            <div>
              <v-btn rounded 
                :small="widthState.small"
                :x-small="widthState.x_small" 
    ...
    <div id="header" class="primary d-flex flex-column align-center justify-space-between secondary--text mb-8">
      <div class="d-flex flex-column align-center">
        <v-img 
          ...
          max-width="250"
          contain/>
        <div class="pt-2 px-10"><span class="text-h4 font-weight-thin">Send audio messages through email</span></div>
      </div>
      <div class="mt-8 mb-6 relative d-flex justify-center">
        <v-img 
          ...
          max-width="70vw"
          class="rounded-lg"/>
        <playback id="playback" class="accent--text d-none d-md-block"/>
        <div class="left">
      ...
      <div id="try-free-section" class="mb-10">
        <div class="pa-2 d-flex justify-center subtitle-1"><span>No attachments, no files, no hassle</span></div>
        <v-btn rounded class="primary--text mx-auto d-flex pa-5">Try For Free Today</v-btn>
      </div>
    </div>

    <div v-show="cardsRenderReady">
      <div class='cards d-flex justify-space-around py-16'>
        <v-card v-for="card in cards" :key="card.key" 
          class='card elevation-4 d-flex flex-column align-center pa-6 relative'>
          <v-chip v-if="card.id === 'card_3'" outlined color="red" id="coming-soon">Coming Soon!</v-chip>
          <v-img 
            v-if="true"
          ...
          <div class="text-caption font-weight-light"><span class="text-h4" style="vertical-align: sub;">${{ card.price }}</span> /Month</div>
          <div class="text-caption font-weight-light py-2">{{ card.blurb }}</div>
          <ul class="font-weight-medium pr-6">
            <li v-for="point in card.points" :key="point.id">{{ point.text }}</li>
          </ul>
        </v-card>
      </div>

      <div id="contact-us-text" class="d-flex justify-center flex-wrap align-center pb-12">
        <span class="primary--text text-center ma-3 font-weight-medium">Let us show you how MicDrop can help your team</span>
        <v-btn rounded class="primary secondary--text pa-5">Contact Us</v-btn>
      </div>
    </div>
    ...
    <div class="footer d-flex flex-wrap justify-center mx-auto">
      ...
      <div class="social text-h4">
        <span class="mdi mdi-linkedin" href="https://www.facebook.com/sendmicdrop"></span>
        <span class="mdi mdi-facebook" href="https://www.linkedin.com/company/micdrop2022/"></span>
      </div>
    </div>

    <div class="accent--text text-caption font-weight-light pb-8 pl-8 pt-4">
      <span class="mdi mdi-copyright mr-2"></span>Copyright, MicDrop LLC, All Rights Reserved
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const cardsRenderReady = computed(() => {
      return IMAGE_LOADED.value.card_1 && IMAGE_LOADED.value.card_2 && IMAGE_LOADED.value.card_3
    })
    ...
    const getImages = async () => {
      try {
        headerBarElements = new ElementArray(...Array.from(
          document.getElementById("header-bar")?.children || []
        ));
    ...
    const getHTMLElements = () => {
      ...
      let cardElements = Array.from(document.getElementsByClassName('card v-card'));
    ...
    const handleScroll = () => {
      ...
      const mainLogoIsVisible = atTop || mainLogo.getBoundingClientRect().bottom - 20 > getHeaderHeight();
    ...
    const cards = ref([
      ...
      },
      {
        ...
        logoName: '',
        price: 0,
        blurb: 'For Business teams looking to improve email efficiency and response rate',
        points: [{
